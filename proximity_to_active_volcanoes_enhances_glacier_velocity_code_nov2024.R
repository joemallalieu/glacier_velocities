...
# Mallalieu J., Barr I.D., Spagnolo M., Mullan D.J., Symeonakis E., Edwards B.R. and Martin M.D. (2024) Proximity to active volcanoes increases glacier velocity. Communications Earth & Environment, 5, 679. https://doi.org/10.1038/s43247-024-01826-5
...
# If you use this R code or the associated data please cite Mallalieu et al. (2024) using the reference above.

# The data for these analyses can be downloaded (in .csv format) from the UK Polar Data Centre:
# Mallalieu J., Barr I.D., Spagnolo M., Mullan D.J., Symeonakis E., Edwards B.R. and Martin M.D. (2024) A database of global glacier and glacierised volcano properties, including measurements of glacier velocity, thickness and climate for the period 2017-2018 (Version 1.0) [Data set]. NERC EDS UK Polar Data Centre. https://doi.org/10.5285/ae909122-b59f-4245-8ce9-f063f837fdf3
# To comply with the terms of the UK Polar Data Centre, several variables in the .csv file had units...
# ...appended to their names in brackets () subsequent to the analysis. Thus, to repeat the analysis...
# ...using this R code you will need to delete the units (including brackets and preceding space)...
# ...from the variable names in the .csv file prior to importing it into R.

# Notes on terminology:
# In the above publication, the terms 'glacier near a volcano' and 'other glacier' correspond to the terms...
# ...'volcanic glacier' and 'non-volcanic glacier' respectively in this code.
...
# Import data
volcglacdata <- read.csv(file = "ADD YOUR FILE PATH TO THE CSV FILE HERE")
View(volcglacdata)
...
volcglacdata$glacier.type <- factor(volcglacdata$glacier.type, levels = c("volcanic glacier", "non-volcanic glacier"))
...
# Calculate confidence intervals for median glacier velocities by glacier type (volcanic/non-volcanic)
near.volc.table <- volcglacdata %>% filter(glacier.type == "volcanic glacier")
near.volc.vels <- near.volc.table$glacier.vel.median[!is.na(near.volc.table$glacier.vel.median)]
sort(near.volc.vels)[qbinom(c(.025,.975), length(near.volc.vels), 0.5)]   # CIs for volcanic glaciers

non.volc.table <- volcglacdata %>% filter(glacier.type != "volcanic glacier")
non.volc.vels <- non.volc.table$glacier.vel.median[!is.na(non.volc.table$glacier.vel.median)]
sort(non.volc.vels)[qbinom(c(.025,.975), length(non.volc.vels), 0.5)]   # CIs for non-volcanic glaciers
...
# Reorder glacier type factor (determines reference factor in linear models)
volcglacdata.inf.sub.scaled.add.nadrop$glacier.type <- factor(volcglacdata.inf.sub.scaled.add.nadrop$glacier.type, levels = c("non-volcanic glacier", "volcanic glacier"))
...
# Reorder glacier type factor (determines reference factor in linear models)
volcglacdata.inf.sub.logthick.scaled.add.nadrop$glacier.type <- factor(volcglacdata.inf.sub.logthick.scaled.add.nadrop$glacier.type, levels = c("non-volcanic glacier", "volcanic glacier"))
...
# Removing the influence of glacier terminus environment on glacier dynamics
# Add glacier terminus type classification into dataset
volcglacdata.inf.sub.logthick.scaled.add.termtype <- cbind(volcglacdata.inf.sub.logthick.scaled.add, volcglacdata.inf$glacier.term.type)
View(volcglacdata.inf.sub.logthick.scaled.add.termtype)

# Rename added column
colnames(volcglacdata.inf.sub.logthick.scaled.add.termtype)[which(names(volcglacdata.inf.sub.logthick.scaled.add.termtype) == "volcglacdata.inf$glacier.term.type")] <- "glacier.term.type"
View(volcglacdata.inf.sub.logthick.scaled.add.termtype)

# Drop data frame rows containing any NAs
volcglacdata.inf.sub.logthick.scaled.add.termtype.nadrop <- drop_na(volcglacdata.inf.sub.logthick.scaled.add.termtype)
View(volcglacdata.inf.sub.logthick.scaled.add.termtype.nadrop)

# Remove all non-terrestrial terminating glaciers
volcglacdata.inf.sub.logthick.scaled.add.termtype.nadrop.terrestrial <- volcglacdata.inf.sub.logthick.scaled.add.termtype.nadrop %>% filter(glacier.term.type == "terrestrial")
View(volcglacdata.inf.sub.logthick.scaled.add.termtype.nadrop.terrestrial)   # NB. 1,924 glaciers classified as calving (i.e. marine or lake), or shelf terminating removed from dataset.

# Reorder glacier type factor (determines reference factor in linear models)
volcglacdata.inf.sub.logthick.scaled.add.termtype.nadrop.terrestrial$glacier.type <- factor(volcglacdata.inf.sub.logthick.scaled.add.termtype.nadrop.terrestrial$glacier.type, levels = c("non-volcanic glacier", "volcanic glacier"))

# Parameterise 3rd model (to test effect of removing all non-terrestrial terminating glaciers).
LM3 <- lm(log10(glacier.vel.median) ~ glacier.type + glacier.thick.median.log10 + glacier.zmed + climate.pca.2017.2018,
          data = (volcglacdata.inf.sub.logthick.scaled.add.termtype.nadrop.terrestrial))

# Output LM3 results
print(summary(LM3), digits = 5)

# Calculate LM3 confidence intervals
print(confint(LM3), digits = 2)

# Output LM3 result as table
LM3rep <- report(LM3)
as.report_table(LM3rep, summary = TRUE)

# Output LM3 standard errors
summary(LM3)$coefficients[, "Std. Error"]

# Calculate percentage of glaciers terminating in a non-terrestrial environment by glacier type
print(no.volcanic.nonterrestrial <- volcglacdata.inf.sub.logthick.scaled.add.termtype.nadrop %>%
        filter(glacier.type == "volcanic glacier", glacier.term.type != "terrestrial") %>% nrow)
print(no.volcanic.terrestrial <- volcglacdata.inf.sub.logthick.scaled.add.termtype.nadrop %>%
        filter(glacier.type == "volcanic glacier", glacier.term.type == "terrestrial") %>% nrow)
print(percent.volcanic.nonterrestrial <- (no.volcanic.nonterrestrial/no.volcanic.terrestrial)*100)   # % of volcanic glaciers terminating in a non-terrestrial setting

print(no.nonvolcanic.nonterrestrial <- volcglacdata.inf.sub.logthick.scaled.add.termtype.nadrop %>%
        filter(glacier.type != "volcanic glacier", glacier.term.type != "terrestrial") %>% nrow)
print(no.nonvolcanic.terrestrial <- volcglacdata.inf.sub.logthick.scaled.add.termtype.nadrop %>%
        filter(glacier.type != "volcanic glacier", glacier.term.type == "terrestrial") %>% nrow)
print(percent.nonvolcanic.nonterrestrial <- (no.nonvolcanic.nonterrestrial/no.nonvolcanic.terrestrial)*100)   # % of non-volcanic glaciers terminating in a non-terrestrial setting
...
as.data.frame(onlyvolc %>% group_by(glacier.distance.to.vent) %>% summarise (n()))   # Counts no. of glaciers within each distance category 
...
# Calculate confidence intervals for median glacier velocities by proximity to volcano (<= 1.0 km/> 1.0 - <= 2.5 km/> 2.5 - <= 5.0 km)
near.volc.vels.near <- near.volc.table %>% filter(glacier.distance.to.vent == "<= 1.0 km")
near.volc.vels.near.vels <- near.volc.vels.near$glacier.vel.median[!is.na(near.volc.vels.near$glacier.vel.median)]
sort(near.volc.vels.near.vels)[qbinom(c(.025,.975), length(near.volc.vels.near.vels), 0.5)]   # CIs for glaciers <= 1.0 km from volcano

near.volc.vels.medium <- near.volc.table %>% filter(glacier.distance.to.vent == "> 1.0 - <= 2.5 km")
near.volc.vels.medium.vels <- near.volc.vels.medium$glacier.vel.median[!is.na(near.volc.vels.medium$glacier.vel.median)]
sort(near.volc.vels.medium.vels)[qbinom(c(.025,.975), length(near.volc.vels.medium.vels), 0.5)]   # CIs for glaciers > 1.0 - <= 2.5 km from volcano

near.volc.vels.far <- near.volc.table %>% filter(glacier.distance.to.vent == "> 2.5 - <= 5.0 km")
near.volc.vels.far.vels <- near.volc.vels.far$glacier.vel.median[!is.na(near.volc.vels.far$glacier.vel.median)]
sort(near.volc.vels.far.vels)[qbinom(c(.025,.975), length(near.volc.vels.far.vels), 0.5)]   # CIs for glaciers > > 2.5 - <= 5.0 km from volcano
...
# Summarise means and sds of unscaled data
unscaled.volconly.means.sds <- as.data.frame(t(onlyvolc.sub %>% summarise(mean(glacier.thick.median, na.rm = T), 
                                                                          mean(glacier.zmed, na.rm = T), mean(climate.pca.2017.2018, na.rm = T),
                                                                          mean(eruption.abp, na.rm = T), mean(no.holocene.eruptions, na.rm = T),
                                                                          sd(glacier.thick.median, na.rm = T), 
...
# Summarise means and sds of scaled data
scaled.volconly.means.sds <- as.data.frame(t(onlyvolc.sub.scaled %>% summarise(mean(glacier.thick.median, na.rm = T), 
                                                                               mean(glacier.zmed, na.rm = T), mean(climate.pca.2017.2018, na.rm = T),
                                                                               mean(eruption.abp, na.rm = T), mean(no.holocene.eruptions, na.rm = T),
                                                                               sd(glacier.thick.median, na.rm = T), 
                                                                               sd(glacier.zmed, na.rm = T), sd(climate.pca.2017.2018, na.rm = T),
...
# Calculate lmm2 standardised residuals
